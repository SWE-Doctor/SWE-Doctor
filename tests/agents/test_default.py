...
def test_cost_limit_trips_on_cost_carrying_format_storm(default_config):
    """A persistent FormatError storm that carries per-turn cost must trip cost_limit,
    not silently spin to step_limit (regression: cost was lost on the raise path)."""
    from minisweagent.exceptions import FormatError

    class StormModel(DeterministicModel):
        """Raises a cost-carrying FormatError on every query (a persistent zero-action storm)."""

        def __init__(self, cost_per_turn=0.4):
        ...
        def query(self, messages, **kwargs):
            raise FormatError(
                {
                    "role": "user",
                    "content": "Please provide EXACTLY ONE action.",
                    "extra": {"interrupt_type": "FormatError", "n_actions": 0, "cost": self._cost_per_turn},
                }
            )
    ...
    agent = DefaultAgent(
        model=StormModel(),
        env=LocalEnvironment(),
        **{**default_config, "cost_limit": 1.0, "step_limit": 10},
    )

    info = agent.run("trigger a format storm")

    assert info["exit_status"] == "LimitsExceeded"
    assert agent.cost == pytest.approx(1.2)  # 3 storm turns * $0.4 billed before tripping
    assert agent.n_calls == 3  # tripped by cost_limit, well before step_limit (10)
